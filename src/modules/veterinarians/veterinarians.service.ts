...
export class VeterinariansService {
  ...
  async findOne(id: string): Promise<any> {
    ...
    const userObj = (user as any).toObject ? (user as any).toObject({ virtuals: true }) : JSON.parse(JSON.stringify(user));
    // Merge all vet fields into User object for client consumption
    userObj.vetLicenseNumber = vet.licenseNumber;
    userObj.vetClinicName = vet.clinicName;
    userObj.vetAddress = vet.clinicAddress;
    userObj.vetSpecializations = vet.specializations;
    userObj.vetYearsOfExperience = vet.yearsOfExperience;
    userObj.vetBio = vet.bio;
    // Use vet's latitude/longitude if available, otherwise keep user's
    if (vet.latitude !== undefined) userObj.latitude = vet.latitude;
    if (vet.longitude !== undefined) userObj.longitude = vet.longitude;
    return userObj;
  }
  ...
  async update(id: string, updateVetDto: UpdateVetDto): Promise<UserDocument> {
    ...
    if (updateVetDto.phoneNumber !== undefined) {
      userFields.phoneNumber = updateVetDto.phoneNumber;
    }
    if (updateVetDto.country !== undefined) {
      userFields.country = updateVetDto.country;
    }
    if (updateVetDto.city !== undefined) {
      userFields.city = updateVetDto.city;
    }
