...
export class SubscriptionsService {
  ...
  async create(
    ...
    // If user has an active or expires_soon subscription, throw error
    if (existingSubscription && 
        (existingSubscription.status === SubscriptionStatus.ACTIVE || 
         existingSubscription.status === SubscriptionStatus.EXPIRES_SOON)) {
    ...
    // If user has a canceled, expired, or pending subscription - we'll reuse it
    // This allows users to resubscribe after cancellation or abandoned payments
    const canceledOrExpiredSubscription = existingSubscription && 
  ...
  /**
   * Update subscription role (vet, trainer, or sitter) after payment
   * Used when user subscribes first, then chooses their professional role
   * For vet and trainer: sets roleApprovalStatus to 'pending' (requires admin approval)
   * For sitter: sets roleApprovalStatus to 'approved' (no approval needed)
   */
  async updateSubscriptionRole(userId: string, role: string): Promise<SubscriptionResponseDto> {
    ...
    if (role !== 'vet' && role !== 'sitter' && role !== 'trainer') {
      throw new BadRequestException('Role must be either "vet", "sitter", or "trainer"');
    }

    // Determine approval status based on role
    // Vet and trainer require admin approval, sitter does not
    const roleApprovalStatus = (role === 'vet' || role === 'trainer') ? 'pending' : 'approved';
    ...
    if (roleApprovalStatus === 'pending') {
      console.log(`⏳ Updated subscription role to ${role} for user ${userId} - awaiting admin approval`);
    } else {
  ...
  async cancel(userId: string): Promise<CancelSubscriptionResponseDto> {
    ...
    // Remove professional profile record based on subscription role
    try {
      ...
      } else if (subscription.role === 'trainer') {
        // Note: Trainer records are managed in trainers module
        // If you have a trainers service with remove method, call it here
        console.log(`✅ Subscription canceled for trainer user ${userId}`);
      }
    } catch (error) {
    ...
    for (const subscription of expiredSubscriptions) {
      ...
      try {
        if (subscription.role === 'vet') {
          ...
          // Note: Trainer records are managed in trainers module
          console.log(
            `✅ Auto-canceled: Subscription canceled for trainer user ${subscription.userId}`,
          );
        }
      } catch (error) {
