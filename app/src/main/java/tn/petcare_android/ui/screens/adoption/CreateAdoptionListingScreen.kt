...
import tn.petcare_android.data.api.CreateAdoptionListingRequest
...
fun CreateAdoptionListingScreen(
    ...
    if (showSuccessDialog) {
        AlertDialog(
            ...
            title = { Text("Success!") },
            text = { Text("Your adoption listing has been created successfully.") },
            confirmButton = {
                TextButton(onClick = {
                    showSuccessDialog = false
                    navController.navigateUp()
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Create Adoption Listing") },
                navigationIcon = {
            ...
            Button(
                onClick = {
                    if (!isSubmitting && !isLoading) {
                        val request = CreateAdoptionListingRequest(
                            petName = petName,
                            species = species,
                ...
                enabled = !isLoading && petName.isNotBlank() && species.isNotBlank()
            ) {
                Text(
                    text = if (isLoading) "Creating..." else "Create Adoption Listing",
